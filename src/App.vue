        ...
        <b-form @submit="onSubmit" @reset="onReset">
          <b-form-group label="Pytanie:" label-for="question">
            <b-form-input id="question" type="text" v-model="question" placeholder="Treść pytania"
              @input="$v.question.$model = $event.trim()" :state="!$v.question.$dirty ? null : !$v.question.$error" />
          ...
          <b-row>
            ...
            <b-alert :show="$v.answers.minLength < 2 || !$v.answers.required" variant="warning">Wymagana liczba
              odpowiedzi: 2
            </b-alert>
            <b-col sm="6" :key="`answ-${index}`" v-for="(answer, index) in answers">
              ...
              <b-form-group :label="`Odpowiedź ${index + 1}`" :label-for="`answer-${index + 1}`">
                ...
                <b-form-input id="`answer-${index+1}`" type="text" v-model.trim="answers[index]"
                  placeholder="Treść odpowiedzi" @input="$v.answers.$each[index].$model = $event.trim()"
          ...
          <section class="buttons">
            ...
            &nbsp; <b-button variant="danger" @click="answers.pop()" :disabled="this.answers.length <= 0">Usuń
              ostatnią
              odpowiedź</b-button>
